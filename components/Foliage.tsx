...
const vertexShader = `
  ...
  void main() {
    ...
    // Swirling orbital motion in chaos mode (when easedProgress is low)
    float chaosAmount = 1.0 - smoothstep(0.0, 0.4, easedProgress);
    if (chaosAmount > 0.0) {
      float swirl = uTime * 0.5 + aRandom * 6.28;
      float radius = length(newPos.xz) * 0.08;
      newPos.x += sin(swirl) * radius * chaosAmount;
      newPos.z += cos(swirl) * radius * chaosAmount;
      newPos.y += sin(swirl * 0.7) * 0.3 * chaosAmount;
    }
...
export const Foliage: React.FC<FoliageProps> = ({ mode, count }) => {
  ...
  // Track mode transition for burst effect
  const transitionRef = useRef({ startTime: 0, lastMode: mode });
  ...
  const { chaosPositions, targetPositions, randoms } = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // 2. Target Positions: Volumetric Cone (Fibonacci Lattice with depth)
      // Normalized height 0 to 1
      const yNorm = i / count;
      const y = yNorm * height;

      // Add radial thickness - particles fill 50-100% of the cone radius for dense volume
      const radiusVariation = 0.5 + Math.random() * 0.5;
      const currentRadius = maxRadius * (1 - yNorm) * radiusVariation;

      // Add slight angular jitter for more organic look
      const angleJitter = (Math.random() - 0.5) * 0.3;
      const angle = 2 * Math.PI * goldenRatio * i + angleJitter;
  ...
  useFrame((state, delta) => {
    if (meshRef.current) {
      ...
      // Detect mode change for burst effect
      if (mode !== transitionRef.current.lastMode) {
        transitionRef.current.startTime = state.clock.elapsedTime;
        transitionRef.current.lastMode = mode;
      }

      // Calculate adaptive lerp speed - fast burst then settle
      const timeSinceSwitch = state.clock.elapsedTime - transitionRef.current.startTime;
      let lerpSpeed: number;
      if (timeSinceSwitch < 0.3) {
        // Fast burst for first 0.3s: start at 15, decay to 6
        lerpSpeed = 15 - timeSinceSwitch * 30;
      } else {
        // Normal speed after settling
        lerpSpeed = 2.5;
      }
