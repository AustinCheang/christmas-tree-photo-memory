...
export const GestureController: React.FC<GestureControllerProps> = ({ onModeChange, currentMode, onHandPosition, onTwoHandsDetected, onSwipe }) => {
  ...
  // Finger curl navigation cooldown
  const curlCooldown = useRef(false);

  // Refs to store latest callbacks (avoid stale closures)
  const onSwipeRef = useRef(onSwipe);
  const onHandPositionRef = useRef(onHandPosition);
  const onTwoHandsDetectedRef = useRef(onTwoHandsDetected);

  // Helper function to detect pinch gesture (thumb and index finger tips close together)
  const detectPinch = (landmarks: any[]): boolean => {
    ...
    // Pinch detected when tips are close (threshold ~0.05 in normalized coordinates)
    ...
    const predictWebcam = () => {
      ...
      if (videoRef.current.videoWidth > 0) { // Ensure video is ready
        ...
        if (result.landmarks && result.landmarks.length > 0) {
          ...
          // Thumb+index curl navigation when two hands are present
          // Mirror-corrected: MediaPipe "Left" = user's right hand in mirrored view
          if (twoHandsDetected && result.handednesses) {
            for (let i = 0; i < result.landmarks.length; i++) {
              const handedness = result.handednesses[i][0].categoryName; // "Left" or "Right"
              const landmarks = result.landmarks[i];
              const isPinched = detectPinch(landmarks);

              if (isPinched && !curlCooldown.current && onSwipeRef.current) {
                // Mirror-corrected: Left detection = right hand = next, Right detection = left hand = previous
                const direction = handedness === 'Left' ? 'right' : 'left';
                console.log(`[Gesture] ${handedness} hand (user's ${handedness === 'Left' ? 'right' : 'left'}) pinch -> ${direction}`);
                onSwipeRef.current(direction);
                curlCooldown.current = true;
                setTimeout(() => { curlCooldown.current = false; }, 500);
                break; // Only process one curl at a time
              }
            }
          }
