...
const MAX_PHOTOS = 50; // Maximum recommended photos for performance
...
const PolaroidItem: React.FC<{ data: PhotoData; mode: TreeMode; index: number }> = ({ data, mode, index }) => {
  ...
  useFrame((state, delta) => {
    ...
    if (isFormed) {
        ...
        // Chaos mode - face toward actual camera position
    ...
    // Scale up photos in chaos mode for better visibility
    const targetScale = isFormed ? 1 : 1.8;
...
export const Polaroids: React.FC<PolaroidsProps> = ({ mode, uploadedPhotos, twoHandsDetected }) => {
  const groupRef = useRef<THREE.Group>(null);
  ...
  const photoData = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // 2. Chaos Position - Spread in front hemisphere for better visibility
      const cR = 15 + Math.random() * 15;
      const cTheta = (Math.random() - 0.5) * Math.PI; // Front hemisphere only (-PI/2 to PI/2)
